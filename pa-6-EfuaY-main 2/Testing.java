    ...
    public static void main(String[] args){
        ...
        for(int i=0; i<tests; i++){
            ...
            if(compare(refTest, outTest)){ // test passed
                ...
                if(i==1 || i==5 || i==6){
                    for(int k=0; k<refTest.size(); k++){
                        String[] refTokens = refTest.get(k).split("\\s+");
                        String[] testTokens = outTest.get(k).split("\\s+");
                        if(refTokens.length != testTokens.length)
                            break;
                        int l=0;
                ...
                if(!dateDiff){
                    ...
                    for(int j=0; j<120; j++)
                        System.out.printf("=");
                    System.out.println();
                    count++; // count the testcase as passed
                }
            }
        }
        if(count == tests){
